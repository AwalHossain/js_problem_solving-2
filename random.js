...
const announce = alert("let's do some work")
// 3. Create a variable and use the prompt() method to collect a number from a visitor
const inputOne = prompt("Input a number?")
const inputTwo = prompt("Input another number?")
// 4. Convert that value from a string to a floating point number
const inputNumberOne = parseInt(inputOne);
const inputNumberTwo = parseInt(inputTwo);
const total = inputNumberOne + inputNumberTwo;
...
let message = `<h1>Math with the Number ${inputNumberOne} and ${inputNumberTwo}</h1> 
      ${inputNumberOne} + ${inputNumberTwo} = ${total};
`
console.log(message)
